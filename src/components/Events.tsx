        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {events.map((event, i) => (
            <motion.div
              ...
              className={`border rounded-sm p-7 bg-card hover:border-primary/30 transition-all duration-300 hover:shadow-md flex flex-col ${
                i === 2 ? 'border-primary/30' : 'border-border'
              }`}
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-1.5 text-xs font-medium text-gold-text">
                  <Calendar className="w-3.5 h-3.5 shrink-0" />
                  <span>{event.date}</span>
                </div>
                {event.capacity && (
                  <span className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-gold-text bg-primary/10 px-2 py-1 rounded-sm">
